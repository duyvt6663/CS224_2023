...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTMCell(embed_size + hidden_size, hidden_size, bias=True) # tentative, will check later
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        # 4. Compute `dec_init_state` = (init_decoder_hidden, init_decoder_cell)
        # Remember that self.encoder has only one layer.
        concat_last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim=1)
        concat_last_cell = torch.cat((last_cell[0], last_cell[1]), dim=1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        # 4. Use torch.stack to convert combined_outputs from a list length tgt_len of tensors shape (b, h)
        # to a single tensor shape (tgt_len, b, h)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        # 3. Compute the attention scores e_t, a Tensor shape (b, src_len)
        dec_hidden = dec_hidden.unsqueeze(2) # (b, h, 1)
        # enc_hiddens_proj is shape (b, src_len, h) -> product with dec_hidden is (b, src_len, 1) 
        # -> squeeze to (b, src_len)
        e_t = torch.bmm(enc_hiddens_proj, dec_hidden).squeeze(2)
        ...
        # 2. Use batched matmul between alpha_t and enc_hiddens to obtain the attention output vector, a_t
        # enc_hiddens is shape (b, src_len, 2h) -> product with alpha_t is (b, 1, 2h) -> squeeze to (b, 2h)
        a_t = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens).squeeze(1)

        # 3. Concatenate dec_hidden with a_t to compute tensor U_t
        # dec_hidden is shape (b, h, 1) -> squeeze to (b, h) -> concat with a_t to (b, 3h)
    ...
    def generate_sent_masks(self, enc_hiddens: torch.Tensor, source_lengths: List[int]) -> torch.Tensor:
        ...
        for e_id, src_len in enumerate(source_lengths):
            enc_masks[e_id, src_len:] = 1 # set the mask to 1 after the source length
