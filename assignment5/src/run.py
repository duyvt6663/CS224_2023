...
if args.function == 'pretrain':
    ...
    tconf = trainer.TrainerConfig(max_epochs=max_epochs, batch_size=batch_size, learning_rate=learning_rate,
        lr_decay=lr_decay, warmup_tokens=warmup_tokens, final_tokens=final_tokens,
        num_workers=num_workers, writer=writer)
    
    trainer_program = trainer.Trainer(model, pretrain_dataset, None, tconf)
    trainer_program.train()

    # TODO [part f] Save the model parameters here
    ...
    if args.reading_params_path is not None:
        # TODO [part f] Load the pretrained model parameters here
        paramters = torch.load(args.reading_params_path)
        model.load_state_dict(paramters)

        # TODO [part f] hyperparameters for finetuning with a pretrained model
        max_epochs=10
    else:
        # TODO [part d] hyperparameters for finetuning without a pretrained model
        max_epochs=75
    
    # common hyperparameters for both finetuning cases
    batch_size=256
    learning_rate=args.finetune_lr
    lr_decay=True
    warmup_tokens=512 * 20
    final_tokens=200 * len(pretrain_dataset) * block_size
    num_workers=4 # 0 for debugging and 4 for gpu
    writer=writer
    ...
    # TODO [part d] [part f] Load finetune_set and finetune the model here
    ...
    trainer_program = trainer.Trainer(model, finetune_dataset, None, tconf)
    trainer_program.train()
        
    # TODO [part d] [part f] Save the model parameters here
    torch.save(model.state_dict(), args.writing_params_path)
