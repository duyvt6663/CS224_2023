...
class DownProjectBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.ln1 = nn.LayerNorm(config.n_embd)
        self.ln2 = nn.LayerNorm(config.n_embd)
        self.cross_attn = attention.CausalCrossAttention(config)
        self.mlp = nn.Sequential(
            nn.Linear(config.n_embd, 4 * config.n_embd),
            nn.GELU(),
            nn.Linear(4 * config.n_embd, config.n_embd),
    ...
    def forward(self, x_input):
        ...
        x_attn = self.cross_attn(self.ln1(x_input), self.ln1(self.C)) # no residual connection
        x_mlp = x_attn + self.mlp(self.ln2(x_attn))
        return x_mlp
        ### END YOUR CODE
...
class UpProjectBlock(nn.Module):
    ...
    def forward(self, y, x_input):
        ...
        ### Hint: Copy over the code from Block and make necessary modifications.
        ### Should be around 3-5 lines.
        x_attn = self.cross_attn(self.ln1(y), self.ln1(x_input)) # no residual connection
        x_mlp = x_attn + self.mlp(self.ln2(x_attn))
        return x_mlp
        ### END YOUR CODE
