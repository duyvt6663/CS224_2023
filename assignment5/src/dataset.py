...
def split_document(document, average_mask_length_ratio=0.25):
    # Length of the entire document
    doc_length = len(document)

    # Calculate average length for masked_content
    average_mask_length = int(doc_length * average_mask_length_ratio)

    # Generate a random length for masked_content around the average length
    # Here, we're using a normal distribution for randomness
    mask_length = int(random.normalvariate(average_mask_length, average_mask_length / 2))
    # Ensuring mask_length is within valid range
    mask_length = max(1, min(mask_length, doc_length))

    # Choose a random starting point for masked_content
    start_index = random.randint(0, doc_length - mask_length + 1)

    # Extract prefix, masked_content, and suffix
    prefix = document[:start_index]
    masked_content = document[start_index:start_index + mask_length]
    suffix = document[start_index + mask_length:]

    return prefix, masked_content, suffix
...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # retrieve the document at self.data[idx]
        document = self.data[idx]

        # truncate the document to a random length between 4 and 7/8 * block_size
        truncated_length = random.randint(4, int(self.block_size*7/8))
        truncated_document = document[:truncated_length]

        # break the document into three substrings prefix, masked_content, suffix
        # where the length of masked_content is random and 1/4 the length of the 
        # truncated document on average
        prefix, masked_content, suffix = split_document(truncated_document)

        # rearrange the substrings into the form prefix MASK_CHAR suffix MASK_CHAR masked_content pads
        masked_string = prefix + self.MASK_CHAR + suffix + self.MASK_CHAR + masked_content + self.PAD_CHAR*(self.block_size - len(prefix) - len(suffix) - len(masked_content) - 2)
