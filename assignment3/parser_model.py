...
class ParserModel(nn.Module):
    ...
    def embedding_lookup(self, w):
        ...
        # get the embedding for each word in the batch
        x = torch.index_select(self.embeddings, 0, w.flatten())
        # reshape the embedding to the correct shape
    ...
    def forward(self, w):
        ...
        embedded_w = self.embedding_lookup(w)
        hidden = F.relu(torch.matmul(embedded_w, self.embed_to_hidden_weight) + self.embed_to_hidden_bias)
        thin_hidden = self.dropout(hidden) # shape: (batch_size, hidden_size)
        logits = torch.matmul(thin_hidden, self.hidden_to_logits_weight) + self.hidden_to_logits_bias
