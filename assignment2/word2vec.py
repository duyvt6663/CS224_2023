...
def naiveSoftmaxLossAndGradient(
    ...
    ### to integer overflow. 
    dot_products = np.dot(outsideVectors, centerWordVec)
    y_hat = softmax(dot_products) # shape = (num words in vocab, )
    loss = -np.log(y_hat[outsideWordIdx])

    gradCenterVec = -outsideVectors[outsideWordIdx] + np.dot(y_hat, outsideVectors)

    # should be (y_hat[outsideWordIdx] - 1)*v_c and y_hat[not outsideWordIdx]*v_c
...
def negSamplingLossAndGradient(
    ...
    # Select the negative samples + outside word
    # Using vectorization for negative sampling
    vectors = outsideVectors[indices]

    # Compute dot products and sigmoid values
    # Dot products - first element is outside word, rest are negative samples
    dot_products = -np.dot(vectors, centerWordVec)
    dot_products[0] = -dot_products[0]  # Flip sign for true outside word
    sigm_vals = sigmoid(dot_products)

    # Calculate loss
    loss = -np.sum(np.log(sigm_vals))

    # Gradient for center word vector
    sigm_vals = 1 - sigm_vals
    sigm_vals[0] = -sigm_vals[0]  # Flip the sign for true outside word contribution
    gradCenterVec = np.dot(sigm_vals, vectors)

    # Efficient Gradient Calculation for outside vectors
    gradOutsideVecs = np.zeros_like(outsideVectors)
    np.add.at(gradOutsideVecs, indices, np.outer(sigm_vals, centerWordVec))
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        ...
        loss_, gradCenterVec, gradOutsideVecs_ = word2vecLossAndGradient(
